...
// We dont get llvm dump after all passes,we will use
// this for debugging purpose
#ifndef NDEBUG
  const char *option=std::getenv("AMD_DEBUG_LLVM_PRINT_AFTER_ALL");
  if (option && (option[0] == '1')) return true;
#endif // !NDEBUG
#endif // AMD_OPENCL
...
bool FunctionPassManagerImpl::run(Function &F) {
  ...
  wasRun = true;
#if defined(AMD_OPENCL) || 1
  // should be removed after fixing the bug with memory leaks in llvm_shutdown 
  // http://llvm.org/bugs/show_bug.cgi?id=14621
  // in order not to duplicate timing report
  TheTimeInfo->printAll();
#endif
  return Changed;
}
