...
#if defined(AMD_OPENCL) || 1
static cl::opt<unsigned>
FullCondEvalThreshold("if-cond-folding-threshold", cl::Hidden, cl::init(20),
       cl::desc(
         "Maximum number of instructions"
         " that may be folded from if condition"));

static bool UseAAInPhiNodeFolding = false;
static cl::opt<cl::boolOrDefault> FlagUseAAInPhiNodeFolding(
  "use-aa-in-phi-node-folding",
  cl::desc("Use AliasAnalysis during two entry phi node folding"),
...
class SimplifyCFGOpt {
  ...
  bool CompareBlock(BasicBlock *Head1,BasicBlock * Head2,BasicBlock *Block1, BasicBlock *Block2);
#endif // AMD_OPENCL
public:
#if defined(AMD_OPENCL) || 1
  SimplifyCFGOpt(const DataLayout *td, const TargetTransformInfo *tti,
          MemoryDependenceAnalysis *mda, AliasAnalysis * aa)
              : TD(td), TTI(tti), MDA(mda), AA(aa) {}
#else
  SimplifyCFGOpt(const DataLayout *td, const TargetTransformInfo *tti)
      : TD(td), TTI(tti) {}
#endif // AMD_OPENCL
...
static bool DominatesMergePoint(Value *V, BasicBlock *BB,
...
#if defined(AMD_OPENCL) || 1
  if (UseAAInPhiNodeFolding) {
    // If we can not analyze memory dependencies and instruction uses memory
    if (!MDA && (I->mayReadFromMemory() || I->mayWriteToMemory()))
      return false;
    ...
    if (MDA && (I->mayReadFromMemory() || I->mayWriteToMemory()))
    {
      ...
      if (memdep.isClobber() || memdep.isDef())
        if (!DominatesMergePoint(memdep.getInst(),
              BB , AggressiveInsts, CostRemaining, MDA))
         return false;
    }

    // Check operand dependencies
    for (User::op_iterator i = I->op_begin(), e = I->op_end(); i != e; ++i)
      if (!DominatesMergePoint(
            *i, BB, AggressiveInsts, CostRemaining, MDA))
        return false;

    // We can hoist this instruction. Care about cost of this hoisting later.
  }
  else {
#endif
  ...
  for (User::op_iterator i = I->op_begin(), e = I->op_end(); i != e; ++i)
    if (!DominatesMergePoint(*i, BB, AggressiveInsts, CostRemaining, MDA))
      return false;

#if defined(AMD_OPECNL) || 1
  }
#endif
...
#if 1 || defined(AMD_OPENCL_BARRIER_CLONE)
    if (barrierCloneOk == false &&
      isa<CallInst>(BBI) && isa<IntrinsicInst>(BBI) == false) {
      return false;
    }
#endif
...
static bool FoldTwoEntryPHINode(PHINode *PN, const DataLayout *TD) {
...
#if defined(AMD_OPENCL) || 1
  // Bug 9325: Remove this patch.
  AMDLLVMContextHook *amdhook = static_cast<AMDLLVMContextHook*>(
    BB->getParent()->getContext().getAMDLLVMContextHook());
  unsigned phiNodeThreshold =
    (amdhook && amdhook->amdoptions.IsGPU) ? 20 : 2;
#endif
  ...
  for (BasicBlock::iterator II = BB->begin(); isa<PHINode>(II);) {
    ...
    if (!DominatesMergePoint(PN->getIncomingValue(0), BB,
                             &AggressiveInsts, MaxCostVal0, MDA) ||
        !DominatesMergePoint(PN->getIncomingValue(1), BB,
                             &AggressiveInsts, MaxCostVal1, MDA))
      return false;
#else
...
#if defined(AMD_OPENCL) || 1
  // Threshold hoisting cost
  // TODO_HSA: Use ComputeSpeculationCost instead of
  //           simple instruction counting
  unsigned HoistingCost = 0;
#endif
...
#if defined(AMD_OPENCL) || 1
  if (UseAAInPhiNodeFolding &&
      HoistingCost >= FullCondEvalThreshold) {
    return false;
  }
#endif
...
#if defined(AMD_OPENCL) || 1
    // Preserves Loop  Metadata
    if (MDNode * MD = BI->getMetadata("LoopPragmaInfo")) {
      PBI->setMetadata("LoopPragmaInfo", MD);
    }
#endif
...
#if defined(AMD_OPENCL) || 1
/// If BB has more than one predecessor that is a conditional branch, attempt
//  to use parallel and/or to simplify CFG.  The return value indicates
...
///    br label %if.end;
///
bool SimplifyCFGOpt::SimplifyParallelAndOr(BasicBlock * BB, IRBuilder<> &Builder, Pass *P) {
  ...
  for (SmallSetVector<BasicBlock*, 16>::iterator PI = Preds.begin(),
    ...
    //    BB2    | 
    //   /   \   |
    //  BB3   \  |        where, BB1, BB2 are conditional branches
    //   \    |  /        BB3 is unconditional branch.
    //    \   | /         BB4 is the BB and the merge.
    //       BB4 <= BB
    //
    ...
    //         /      |
    //        |      BB2
    //        \    /    |   where BB1, BB2 are conditional branches,
    // BB =>    BB3     |  BB3 is unconditiona branch and BB,
    //           \     /   BB4 is the merge.
    //             BB4
    //
    // Corresponding source code:
    ...
    //   statement;  // BB3
    //
    // We identify such cases from the common successor (BB) of conditional 
    // branches.  In Case 1, BB (BB4) has an unconditional branch (BB3) as 
    // its predecessor. In Case 2, BB only has conditional branches as its 
    // predecessors.
    ...
    if (PBI->isUnconditional()) {
      ...
      // is also a predecessor of BB (BB4) and should not have 
      // address-taken.  There should be only one unconditional branch
      // in the predecessors.
    ...
    // Only conditional branches are allowed beyond this.
    if (!PBI->isConditional()) return false;

    // Condition's unique use should be the branch instruction.
    Value * PC = PBI->getCondition();
    if (!PC || (PC->getNumUses() != 1)) return false;

    if (PP && (Preds.count(PP) != 0)) {
      // These are internal condition blocks to be merged from, e.g.,
      // BB2 in both cases. 
      // Should not be address-taken.
      if (Pred->hasAddressTaken()) return false;

      // Instructions in the internal condition blocks should be safe 
      // to hoist up.
      for (BasicBlock::iterator BI = Pred->begin(),
         BE = PBI; BI != BE; ) {
        Instruction * CI = BI++;
      ...
      // both cases.
      if (FCond) return false;
      FCond = Pred;
    }

    // The terminator must have exactly two successors.
    if (PTI->getNumSuccessors() != 2) return false;
    ...
    // PS is the successor who is not BB. Check successors to identify
    // Identify the last conditional branch.
    if (Preds.count(PS) == 0) {
      // Case 2.
      // BB must have an unique successor.
      TerminatorInst * TBB = BB->getTerminator();
      if (TBB->getNumSuccessors() != 1) return false;
...
/// Compare blocks from two if-regions, where Head1 is the head of the 1st
/// if-region, Head2 is the head of the 2nd if-region, Block1 is a block
/// in the 1st if-region to compare, Block2 is a block in the 2nd if-region
/// to compare.  Return true if the blocks have identical instructions
/// that do not alias with Head2.
///
bool SimplifyCFGOpt::CompareBlock(BasicBlock * Head1, BasicBlock * Head2, BasicBlock * Block1, BasicBlock * Block2)
{
  TerminatorInst * PTI2 = Head2->getTerminator();
  Instruction * PBI2 = Head2->begin();

  if (Block1 == Head1) {
    if (Block2 != Head2) return false;
  }
    ...
    while (1) {
      ...
      if (!iter1->isIdenticalTo(iter2)) return false;

      if (iter1->mayReadFromMemory() || iter1->mayWriteToMemory()) {
        for (BasicBlock::iterator BI = PBI2,
          BE = PTI2; BI != BE; BI++) {
          if (BI->mayReadFromMemory() || BI->mayWriteToMemory()) {
            if (!AA || AA->alias(iter1, BI)) return false;
          }
        }
      }
      iter1++;
...
/// Check whether BB is the merge block of a if-region.  If yes, check
/// whether there exists an adjacent if-region upstream and the two
/// if-regions can be merged, e.g.,
//
/// From:
/// if (a) 
///   statement;
...
///   statement;
///
bool SimplifyCFGOpt::MergeIfRegion(BasicBlock * BB, IRBuilder<> &Builder, Pass *P) {
  BasicBlock *IfTrue2, *IfFalse2;
  Value * IfCond2 = GetIfCondition(BB, IfTrue2, IfFalse2);
  if (!IfCond2) return false;

  Instruction * CInst2 = dyn_cast<Instruction>(IfCond2);
  if (!CInst2) return false;

  BasicBlock * Head2 = CInst2->getParent();
  if (Head2->hasAddressTaken()) return false;

  BasicBlock *IfTrue1, *IfFalse1;
  Value * IfCond1 = GetIfCondition(Head2, IfTrue1, IfFalse1);
  if (!IfCond1) return false;

  Instruction * CInst1 = dyn_cast<Instruction>(IfCond1);
  if (!CInst1) return false;

  BasicBlock * Head1 = CInst1->getParent();
  TerminatorInst * PTI2 = Head2->getTerminator();
  Instruction * PBI2 = Head2->begin();

  if (!CompareBlock(Head1, Head2, IfTrue1, IfTrue2)) return false;

  if (!CompareBlock(Head1, Head2, IfFalse1, IfFalse2)) return false;

  // Check whether Head2 has side-effect and is safe to speculate.
  for (BasicBlock::iterator BI = PBI2,
    BE = PTI2; BI != BE; BI++) {
    Instruction * CI = BI;
    if (isa<PHINode>(CI) || CI->mayHaveSideEffects()
...
#if defined(AMD_OPENCL) || 1
  switch (FlagUseAAInPhiNodeFolding)
  {
  case cl::BOU_UNSET:
    UseAAInPhiNodeFolding = AMDOptions::isTargetHSAIL(BB);
    break;
  case cl::BOU_TRUE:
    UseAAInPhiNodeFolding = true;
    break;
  case cl::BOU_FALSE:
    UseAAInPhiNodeFolding = false;
    break;
  }
#endif
...
#if defined(AMD_OPENCL) || 1
  // This should be merged with llvm commit r187278
...
#if defined(AMD_OPENCL) || 1
      Changed |= FoldTwoEntryPHINode(PN, TD, MDA);
#else
...
// We have a lot of calls to SimpifyCFG
// It would be terrible to wrap them all in AMD_HSAIL macros
// One static variable wouldn't hurt

bool
llvm::SimplifyCFG(
  BasicBlock *BB, const DataLayout *TD/*=0*/, const TargetTransformInfo *TTI,
  MemoryDependenceAnalysis *MDA/*=0*/,AliasAnalysis * AA /*=0*/) {
  static MemoryDependenceAnalysis *SavedMDA = 0;

  if (SavedMDA == 0 || (SavedMDA != MDA && MDA != 0))
    SavedMDA = MDA;

  return SimplifyCFGOpt(TD, TTI, SavedMDA, AA).run(BB);
}
