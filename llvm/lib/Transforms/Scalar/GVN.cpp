...
#if defined (AMD_OPENCL_MEMCOMBINE)
// I-Jui: changes to make GVN able to eliminate redundant vector loads after
// inserting a vector load in AMDMemoryCombine.cpp

#include "llvm/Analysis/ValueTracking.h"
namespace llvm {
  namespace VEAA {
    extern int64_t GetIndiceDifference(
...
// return if the load is loading an element from a vector
bool isLoadingVectorElem(Value *StoredVal, const Type *LoadedTy)
{
  const Type *StoredValTy = StoredVal->getType();
  const VectorType *StoredVectValTy =dyn_cast<VectorType>(StoredValTy);
  // does the loaded type agree with vector element type?
  return (StoredVectValTy && StoredVectValTy->getElementType() == LoadedTy);
}

// if the stored value is a vector, and the load is loading an element from
// that vector, create an ExtractElement instruction to replace the load
static Value *ExtractVecElemForLoad(Value *StoredVal,
                                    Type *LoadedTy,
                                    Instruction *InsertPt,
                                    const DataLayout &DL,
                                    unsigned Offset)
{
  // FIXME: check if Stored type is vector of LoadedTy. Insert bitcast if
  // necessary
  // TODO: obtain the element index by dividing GEP1BaseOffset with element
  // type size
  // and TODO: look at data layout?
  assert(isLoadingVectorElem(StoredVal, LoadedTy) && "not loading vector elem");
  assert(LoadedTy->isSized() && "Cannot decide element size");

  // Check whether element of store value size
  int64_t elm_sz = DL.getTypeAllocSize(LoadedTy);
  int64_t VecElmIdx = (Offset+elm_sz-1)/elm_sz;

  DEBUG(errs() << "GVN TRYING TO EXTRACT VECTOR ELEMENT FROM VAL: "
               << *StoredVal <<"\n"
               << "Element idx = " << VecElmIdx << "\n");
  ExtractElementInst *eleminst
    = ExtractElementInst::Create(
        StoredVal,
        ConstantInt::get(Type::getInt32Ty(StoredVal->getContext()), VecElmIdx),
        "v4_ld_gvn",
        InsertPt);
  return eleminst;
}
...
#if defined (AMD_OPENCL_MEMCOMBINE)
  if (isLoadingVectorElem(StoredVal, LoadedTy)) {
    Value *result = ExtractVecElemForLoad(StoredVal, LoadedTy, InsertPt, TD, 0);
...
#if defined (AMD_OPENCL_MEMCOMBINE)
  // Skip bitcasts/shifts if:
  // 1) source value is a vector load AND
  // 2) use is another scalar load of the same type
  // In this case, CoerceAvaliableValueToLoadType
  // will generate correct ExtractElement instruction
  // so bit casts and shifts are not necessary
  if (isLoadingVectorElem(SrcVal, LoadTy)) {
    Value *result = ExtractVecElemForLoad(SrcVal, LoadTy, InsertPt, TD, Offset);
    if (result) return result;
    // if failed to generate ExtractElement, fallthru ...
  }
#endif
...
bool GVN::processNonLocalLoad(LoadInst *LI) {
  ...
  // We don't want to simplify loads 
  // if we will hoist them in different empty basic block.
  // This handles only the case when we have 
  // splitted critical edge at the end of the loop.
  if (UnavailableBlocks[0]->getFirstNonPHIOrDbg() ==
      UnavailableBlocks[0]->getTerminator())
  {
    // See if empty_block is in the middle of critical edge in the loop
    BasicBlock *pred = UnavailableBlocks[0]->getSinglePredecessor(), 
      *succ;

    succ_iterator begin = succ_begin(UnavailableBlocks[0]), 
      end = succ_end(UnavailableBlocks[0]);
    if (begin == end)
      succ = 0;
    else
    {
    ...
    if (succ != 0 && pred != 0 && succ == pred)
      return false;
  }
#endif // defined(AMD_OPENCL)
