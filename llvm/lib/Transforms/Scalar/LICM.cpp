...
#if defined(AMD_OPENCL) || 1
    LivenessAnalysis *LA;
    int  AvailableRegs;      // -1: not used; otherwise, the number of
                             // available registers.
#endif
...
bool LICM::runOnLoop(Loop *L, LPPassManager &LPM) {
  ...
  // Check this
#if defined(AMD_OPENCL) || 1
  LA = &getAnalysis<LivenessAnalysis>();

  // Recompute it always. TODO: need to update liveness
  LA->releaseMemory();

  LA->calculateLiveness(*(L->getHeader()->getParent()));
  if (Preheader && LA->isLivenessValid()) {
    AvailableRegs = LA->getBase()->getNumAvailRegs() -
                    LA->getBase()->getNLRBBOut(Preheader);
    if (AvailableRegs < 0) {
      AvailableRegs = 0;
    }
  }
...
#if defined(AMD_OPENCL) || 1
        if ((AvailableRegs >= 0) &&  
            LA->getBase()->isValueInBBOut(&I, Preheader)) {
          if (AvailableRegs > 0) {
            hoist(I);
            --AvailableRegs;
          } else {
            DEBUG( dbgs()
              << "LICM hoisting inst: " << I 
              << " not performed due to reg pressure(liveness).\n");
          }
        }
        else
#endif
