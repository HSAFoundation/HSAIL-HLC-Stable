...
void llvm::initializeScalarOpts(PassRegistry &Registry) {
  ...
  initializeMemCombinePass(Registry);
  initializeScalarReplArrayElemPass(Registry);
#endif
#if 1 || defined(AMD_OPENCL)
  initializeVectorCoarseningPass(Registry);
  initializeAMDSimplifyLibCallsPass(Registry);
#endif
}
...
#if 1 || defined(AMD_OPENCL_MEMCOMBINE)
void LLVMAddAMDMemCombinePass(LLVMPassManagerRef PM, unsigned maxVecGen,
                              unsigned minAlign)
{
  unwrap(PM)->add(createAMDMemCombinePass(maxVecGen, minAlign));
}
#endif

#if 1 || defined(AMD_OPENCL)
void LLVMAddAMDScalarReplArrayElemPass(LLVMPassManagerRef PM,
                                       unsigned bankWidth,
                                       unsigned halfWavefront,
                                       unsigned SRAEThreshold)
{
  unwrap(PM)->add(createAMDScalarReplArrayElemPass(bankWidth,
                                                   halfWavefront,
                                                   SRAEThreshold));
}
#endif
