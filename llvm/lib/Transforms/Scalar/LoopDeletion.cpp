...
bool LoopDeletion::runOnLoop(Loop* L, LPPassManager& LPM) {
  ...
  if (LoopPragmaInfo *PI = L->getPragmaInfo()) {
    ...
    preheader->getParent()->getContext().getAMDLLVMContextHook());
    if (PI->hasPragmaUnrollCount() && amdhook 
        && amdhook->LLVMBuildLog) {
      ...
      amdhook->LLVMBuildLog->append(
        ") ignored because this loop is dead and deleted.\n");
    }
    delete PI;
    L->setPragmaInfo(NULL);
  }
#endif
