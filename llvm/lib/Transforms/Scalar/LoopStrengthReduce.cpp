...
#if defined(AMD_OPENCL) || 1
// Maximum number of registers allowed to recalculate for each formula.
static cl::opt<unsigned> MaxRegRecalc(
   "lsr-max-reg-recalc", cl::Hidden,
   cl::desc(
     "Maximum number of registers allowed to recalculate for each formula"),
   cl::init(4));

static bool CheckRegRecalc = false;
static cl::opt<cl::boolOrDefault> FlagCheckRegRecalc(
   "lsr-check-reg-recalc", cl::Hidden,
   cl::desc(
     "Check for maximum number of registers allowed to recalculate for each formula"),
   cl::init(cl::BOU_UNSET));

// This option shall not be needed as soon as AddRec cost is properly
// calculated by the LLVM, see Cost::RateRegister() below.
static bool IgnoreAddRecCost = false;
static cl::opt<cl::boolOrDefault> FlagIgnoreAddRecCost(
   "lsr-ignore-addrec-cost", cl::Hidden,
   cl::desc("Ignore addrec cost in lsr solution comparison"),
   cl::init(cl::BOU_UNSET));

static bool CheckLoserOnTie = false;
static cl::opt<cl::boolOrDefault> FlagCheckLoserOnTie(
   "lsr-check-loser-on-tie", cl::Hidden,
   cl::desc("Do not pick lose solution on cost tie"),
   cl::init(cl::BOU_UNSET));
#endif // AMD_OPENCL
...
#if defined(AMD_OPENCL) || 1
          const SCEVAddRecExpr * A = dyn_cast<SCEVAddRecExpr>(Quotient);
          if (A) {
            // If the scale expression is a SCEVAddRecExpr, and it is the
            // same as the value expression of a PHI node, avoid creating
            // a formulae.  Otherwise if this formulae is chosen as the
            // best solution, the PHI node value will be reused and LSR
            // will become a NO-OP.
            const Loop *L = A->getLoop();
            BasicBlock *LatchBlock = L->getLoopLatch();
            bool Skip = false;
            for (BasicBlock::iterator I = L->getHeader()->begin();
             PHINode *PN = dyn_cast<PHINode>(I); ++I) {
             if (SE.isSCEVable(PN->getType())
                && (SE.getEffectiveSCEVType(PN->getType())
                        == SE.getEffectiveSCEVType(Quotient->getType()))
                && (SE.getSCEV(PN) == Quotient)) {
                    Skip = true;
                    break;
                }
             }
             if (Skip)
                ...
                // If the step is not a constant, avoid en-registering
                // this expression.  Otherwise a new IV will be created
                // for this expression, and the reuse of the new IV
                // for this expression effectively suppresses LSR.
...
void LSRInstance::SolveRecurse(SmallVectorImpl<const Formula *> &Solution,
  ...
  for (SmallVectorImpl<Formula>::const_iterator I = LU.Formulae.begin(),
...
#if defined(AMD_OPENCL) || 1
        ++num_regs_recalc;
        if (!CheckRegRecalc || (num_regs_recalc >= MaxRegRecalc))
#endif
    ...
    if ((NewCost < SolutionCost) ||
        (CheckLoserOnTie && (NewCost == SolutionCost) && !NewCost.isLoser())) {
#else
    if (NewCost < SolutionCost) {
#endif // AMD_OPENCL
...
#if defined(AMD_OPENCL) || 1
  const TargetPassConfig& PC = P->getAnalysis<TargetPassConfig>();

  switch (FlagCheckRegRecalc) {
  case cl::BOU_UNSET: CheckRegRecalc = PC.getLsrCheckRegRecalc(); break;
  case cl::BOU_TRUE:  CheckRegRecalc = true; break;
  case cl::BOU_FALSE: CheckRegRecalc = false; break;
  }

  switch (FlagIgnoreAddRecCost) {
  case cl::BOU_UNSET: IgnoreAddRecCost = PC.getLsrIgnoreAddRecCost(); break;
  case cl::BOU_TRUE:  IgnoreAddRecCost = true; break;
  case cl::BOU_FALSE: IgnoreAddRecCost = false; break;
  }

  switch (FlagCheckLoserOnTie) {
  case cl::BOU_UNSET: CheckLoserOnTie = PC.getLsrCheckLoserOnTie(); break;
  case cl::BOU_TRUE:  CheckLoserOnTie = true; break;
  case cl::BOU_FALSE: CheckLoserOnTie = false; break;
  }
#endif
