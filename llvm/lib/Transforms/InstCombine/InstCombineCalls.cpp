...
namespace {
class InstCombineAMDLibCalls : public AMDLibCalls {
  InstCombiner *IC;
protected:
...
public:
  InstCombineAMDLibCalls(InstCombiner *IC) : IC(IC), NewInstruction(0) { }
...
#if 1 || defined(AMD_OPENCL)
  // Todo:  avoid creating simplifier for each call.
  AMDLLVMContextHook *amdhook = static_cast<AMDLLVMContextHook*>(
    CI->getParent()->getParent()->getContext().getAMDLLVMContextHook());
  if (!amdhook || !amdhook->amdoptions.OptSimplifyLibCall) {
    return NULL;
  }

  InstCombineAMDLibCalls Simplifier(this);
  if (amdhook->amdoptions.UnsafeMathOpt)
    Simplifier.enableUnsafeMathOpt();
  if (amdhook->amdoptions.NoSignedZeros)
    Simplifier.enableNoSignedZeros();
  if (amdhook->amdoptions.FiniteMathOnly)
    Simplifier.enableFiniteMathOnly();
  if (amdhook->amdoptions.FastRelaxedMath)
    Simplifier.enableFastRelaxedMath();
