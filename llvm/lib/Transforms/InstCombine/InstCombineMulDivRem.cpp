...
Instruction *InstCombiner::visitFDiv(BinaryOperator &I) {
  ...
  if (ConstantFP *Op1C = dyn_cast<ConstantFP>(Op1)) {
    ...
    else if (EnableUnsafeFPMath && (Op1F.getCategory() == APFloat::fcNormal)) {
      APFloat Reciprocal(Op1F.getSemantics(), 1ULL);
      Reciprocal.divide(Op1F, APFloat::rmNearestTiesToEven);
      if (Reciprocal.getCategory() == APFloat::fcNormal) {
        ConstantFP *RFP = ConstantFP::get(Builder->getContext(), Reciprocal);
        return BinaryOperator::CreateFMul(Op0, RFP);
      }
    }
  ...
  if (Value *V = AMDIC.SimplifyFDiv(&I)) {
    return ReplaceInstUsesWith(I, V);
  }
#endif
  ...
  if (EnableUnsafeFPMath) {
    ...
    unsigned UsageOfDivisorCount = 0;
    for (Value::use_iterator i = Op1->use_begin(), e = Op1->use_end(); i != e 
	   && UsageOfDivisorCount <= 1; ++i)
      if (BinaryOperator * instr = dyn_cast<BinaryOperator>(*i))
	if (instr->isSameOperationAs(&I) && instr->getOperand(1) == Op1 
	    && (&I == instr || DT->dominates(&I, instr) || DT->dominates(instr, &I)))
	  ++UsageOfDivisorCount;
    // check another source for optimization -- loops
    if (UsageOfDivisorCount <= 1) { 
      LoopInfo* LI = &getAnalysis<LoopInfo>();
      if (Loop* L = LI->getLoopFor(I.getParent()))
	if (L->isLoopInvariant(Op1))
	  UsageOfDivisorCount = 100; // Only more than one uses matters
    }

    if (UsageOfDivisorCount > 1) {
      BinaryOperator * OneDivOp1 = 0;
      ConstantFP * OneFP = ConstantFP::get(Builder->getContext(), 
					   APFloat(Op0->getType()->isFloatTy() ? 
						   APFloat::IEEEsingle : 
						   APFloat::IEEEdouble, 1));
      OneDivOp1 = BinaryOperator::CreateFDiv(OneFP, Op1);
      OneDivOp1->insertBefore(&I);
      Worklist.Add(OneDivOp1);
      return BinaryOperator::CreateFMul(Op0, OneDivOp1);
    }
  }
#endif // AMD_OPENCL
