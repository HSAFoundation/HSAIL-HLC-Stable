...
#if defined(AMD_OPENCL) || 1
  // Record promoted values to avoid infinite loop in visitGEP and visitADD
  DenseSet<const Value *> PromotedValues;
#endif
...
#if 1 || defined(AMD_OPENCL)
  InstCombiner(bool optEnableUnsafeFPMath = false) :  FunctionPass(ID), TD(0), 
    Builder(0), AMDIC(), EnableUnsafeFPMath(optEnableUnsafeFPMath) {
#else
...
#if defined(AMD_OPENCL) || 1
  bool IsNonNegative(const Value *V) const;
#endif
