...
static cl::opt<bool>
DoInt32To64Promotion("int32to64-promotion",
          cl::desc("Enable optimization that may promote"
                   " 32-bit integers to 64 bit integers"),
          cl::init(true));
...
#if defined(AMD_OPENCL) || 1
  // Replace following common pattern in addres calculation
  //   %tmp8 = add nsw i32 %conv, <constant>
  //   %conv9 = sext i32 %tmp8 to i64
  //   %arrayidx10 = getelementptr i32 addrspace(1)* %rowDelimiters, i64 %conv9
  // With:
  //   %tmp8 = sext i32 %conv to i64
  //   %conv9 = add nsw i64 %tmp8, <constant>
  //   %arrayidx10 = getelementptr i32 addrspace(1)* %rowDelimiters, i64 %conv9
  // It will improve CSE and will encourage code generator to
  // generate effective address calculation:
  //   ld_global_u64 $c1, [$d1 + <constant> * 4]
  // This transformation is only correct for operations with NSW flag
  // since we have undefined overflow behavior for them
  
  if (DoInt32To64Promotion) {
    SExtInst *sext = NULL;
    unsigned gep_op_idx = 1;

    // Skip zero indexes
    for (; gep_op_idx < GEP.getNumIndices() + 1; ++gep_op_idx)
    {
      ConstantInt *C = dyn_cast<ConstantInt>(GEP.getOperand(gep_op_idx));
      if (C && C->isZero())
        continue;
      sext = dyn_cast<SExtInst>(GEP.getOperand(gep_op_idx));
      break;
    }

    if (sext)
    {
      if (sext->getSrcTy()->isIntegerTy(32) && 
          sext->getDestTy()->isIntegerTy(64) && 
          sext->hasOneUse())
      {
        BinaryOperator *add = dyn_cast<BinaryOperator>(sext->getOperand(0));

        if (add && 
            add->getOpcode() == Instruction::Add &&
            isa<ConstantInt>(add->getOperand(1)) &&
            add->hasOneUse() &&
            (add->hasNoSignedWrap() || add->hasNoUnsignedWrap()))
        {
          // Check if this transformation is profitable
          bool is_profitable = false;

          // TODO_HSA: Remove this once we will have 
          // proper 32 bit pointers for this segments
          #define HSAIL_PRIVATE_ADDRESS 0
          #define HSAIL_GROUP_ADDRESS 3 
          
          if (GEP.getPointerAddressSpace() == HSAIL_PRIVATE_ADDRESS ||
              GEP.getPointerAddressSpace() == HSAIL_GROUP_ADDRESS)
          {
            // It is always profitable to promote add for this addrspaces
            // because it will be packed into address calculation 
            // inside load or store instruction
            is_profitable = true;
          }
          else
          {
            // This transformation is only profitable 
            // if there is multiple calculations with same base address
            const int DfsThreshold = 5;
            Value *base_address = add->getOperand(0);

            int df_cnt = 0;
            for (df_iterator<Value *> df_it = df_begin(base_address), 
                                      df_it_end = df_end(base_address); 
                  df_it != df_it_end && df_cnt < DfsThreshold; 
                  ++df_it, ++df_cnt)
            {
              if (*df_it != &GEP &&
                  isa<GetElementPtrInst>(*df_it))
              {
                is_profitable = true;
                break;
              }
          ...
          #undef HSAIL_PRIVATE_ADDRESS
          #undef HSAIL_GROUP_ADDRESS
          ...
          if (is_profitable)
          {
            ...
            sext->setOperand(0, add->getOperand(0));

            BinaryOperator *new_add = BinaryOperator::Create(Instruction::Add, 
              sext, 
              ConstantExpr::getCast(Instruction::SExt, 
                cast<Constant>(add->getOperand(1)), sext->getType()), 
              "", &GEP);
            new_add->setHasNoSignedWrap(add->hasNoSignedWrap());
            new_add->setHasNoUnsignedWrap(add->hasNoUnsignedWrap());
            GEP.setOperand(gep_op_idx, new_add);

            PromotedValues.insert(new_add);
...
#if defined(AMD_OPENCL) || 1
FunctionPass *llvm::createInstructionCombiningPass(bool EnableUnsafeFPMath) {
  return new InstCombiner(EnableUnsafeFPMath);
}
#endif
