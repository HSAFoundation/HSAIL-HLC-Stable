...
static cl::opt<bool>
IgnoreSignedOverflow("ignore-signed-overflow",
          cl::desc("Enable optimizations that may ignore signed int overflow"),
          cl::init(true));
...
#if defined(AMD_OPENCL) || 1
/// A signed integer will not be negative if only assigned:
/// -  A result of add, mul or sdiv with non-negtive non-variable operands
/// -  A result of any shift of a non-negative value
/// -  A non-negative constant
/// This employs the fact we are not required to handle signed integer overflow
/// according to the standard.
/// The above list can be generally extended.
static bool IsNonNegativeValue(const Value *V, unsigned DepthThresh,
                               DenseMap<const Value*,unsigned>& Visited) {
  if (Visited.find(V) != Visited.end()) // already been there
    return true;
  if (DepthThresh-- == 0) return false; // prevent too deep recursion
  Visited.insert(std::make_pair(V, Visited.size()));
  ...
  if (const Instruction *I = dyn_cast<Instruction>(V)) {
    ...
    case Instruction::Shl:
      if (!IgnoreSignedOverflow) return false;
    case Instruction::LShr:
    case Instruction::AShr:
      return IsNonNegativeValue(I->getOperand(0), DepthThresh, Visited);
    case Instruction::Mul:
    case Instruction::Add:
      if (!IgnoreSignedOverflow) return false;
    case Instruction::SDiv:
      return IsNonNegativeValue(I->getOperand(0), DepthThresh, Visited) &&
             IsNonNegativeValue(I->getOperand(1), DepthThresh, Visited);
    case Instruction::PHI:
      if (const PHINode *PHI = dyn_cast<PHINode>(I)) {
        for( unsigned i=0, e = PHI->getNumIncomingValues(); i < e; i++ )
          if (!IsNonNegativeValue(PHI->getIncomingValue(i), DepthThresh,
                                  Visited))
            return false;
        return true;
      }
...
bool InstCombiner::IsNonNegative(const Value *V) const {
  const unsigned DepthThresh = 3;
  DenseMap<const Value*, unsigned> Visited;
  return ::IsNonNegativeValue(V, DepthThresh, Visited);
}
#endif // AMD_OPENCL
...
#if defined(AMD_OPENCL) || 1
      // We can process unsigned comparison of sext and zext provided sext is
      // non-negative
      if (isSignedCmp) return 0;
      if (IsNonNegative(isSignedExt ? LHSCIOp : RHSCIOp)) {
