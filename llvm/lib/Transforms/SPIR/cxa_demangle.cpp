...
class ___static_cast
    : public __node
{
public:

    ___static_cast(__node* op1, __node* op2)
    {
        __left_ = op1;
        __right_ = op2;
    }
...
public:
    ...
    {
        __left_ = type;
        __size_ = __left_->is_function() ? cv << 16 : cv;
    }
    ...
    virtual size_t first_size() const
    {
        ...
        if (__size_ & 16)
            s += sizeof(" &&")-1;
        if (__size_ & 32)
            s += sizeof(" read_only")-1;
        if (__size_ & 64)
            s += sizeof(" write_only")-1;
        if (__size_ & 128)
            s += sizeof(" read_write")-1;
        if (__size_ & 256)
            s += sizeof(" addrspace(0)")-1;
        if (__size_ & 512)
            s += sizeof(" addrspace(1)")-1;
        if (__size_ & 1024)
            s += sizeof(" addrspace(2)")-1;
        if (__size_ & 2048)
            s += sizeof(" addrspace(3)")-1;
        if (__size_ & 4096)
            s += sizeof(" addrspace(4)")-1;
        if (__size_ & 8192)
            s += sizeof(" addrspace(5)")-1;
        return s;
    }
    virtual char* first_demangled_name(char* buf) const
    {
        ...
        if (__size_ & 32) 
        {
          const size_t n = sizeof(" read_only") - 1;
          strncpy(buf, " read_only", n);
          buf += n;
        }
        if (__size_ & 64) 
        {
          const size_t n = sizeof(" write_only") - 1;
          strncpy(buf, " write_only", n);
          buf += n;
        }
        if (__size_ & 128)
        {
          const size_t n = sizeof(" read_write") - 1;
          strncpy(buf, " read_write", n);
          buf += n;
        }
        if (__size_ & 256) 
        {
          const size_t n = sizeof(" addrspace(0)") - 1;
          strncpy(buf, " addrspace(0)", n);
          buf += n;
        }
        if (__size_ & 512) 
        {
          const size_t n = sizeof(" addrspace(1)") - 1;
          strncpy(buf, " addrspace(1)", n);
          buf += n;
        }
        if (__size_ & 1024) 
        {
          const size_t n = sizeof(" addrspace(2)") - 1;
          strncpy(buf, " addrspace(2)", n);
          buf += n;
        }
        if (__size_ & 2048) 
        {
          const size_t n = sizeof(" addrspace(3)") - 1;
          strncpy(buf, " addrspace(3)", n);
          buf += n;
        }
        if (__size_ & 4096) 
        {
          const size_t n = sizeof(" addrspace(4)") - 1;
          strncpy(buf, " addrspace(4)", n);
          buf += n;
        }
        if (__size_ & 8192) 
        {
          const size_t n = sizeof(" addrspace(5)") - 1;
          strncpy(buf, " addrspace(5)", n);
          buf += n;
        }
        return buf;
    }
...
class ___wchar_t : public __node
{
...
class ___wchar_t_literal
    : public __node
{
public:
    explicit ___wchar_t_literal(const char* __first, const char* __last)
    {
        __name_ = __first;
...
#define BUILTIN_TYPE(A) \
class __##A : public __node \
{ \
protected: \
  static const size_t n = sizeof(#A) - 1; \
public: \
    virtual size_t first_size() const {return n;} \
    virtual char* first_demangled_name(char* buf) const \
    {\
...
BUILTIN_TYPE(image1d_t);
BUILTIN_TYPE(image1d_buffer_t);
BUILTIN_TYPE(image1d_array_t);
BUILTIN_TYPE(image2d_t);
BUILTIN_TYPE(image2d_buffer_t);
BUILTIN_TYPE(image3d_t);
BUILTIN_TYPE(event_t);
BUILTIN_TYPE(sampler_t);
BUILTIN_TYPE(spir_size_t);
BUILTIN_TYPE(spir_ptrdiff_t);

// Allowed element type for OpenCL vector type
//                ::= b    # bool
//                ::= c    # char
//                ::= a    # signed char
//                ::= h    # unsigned char
...
//                ::= d    # double
//                ::= Dh   # half
//TODO: this is a quick workaround. still needs a complete fix.
class __ocl_vector
    : public __node
{
public:
    __ocl_vector(const char* base_type, size_t vector_size)
    {
       sprintf(_buf, "%s%lu", base_type, (unsigned long)vector_size);
       _size = strlen(_buf);
    }
private:
    ...
    static void init_type_map()
    {
        ...
        for (size_t i = 0; i < table_size/2; ++i) {
            _type_map[std::string(table[2*i])] = std::string(table[2*i+1]);
        }
    }
public:
    // return NULL if mangled type is not valid element type
    static const char* map_element_type(const char* mangled,
            const char*& end_of_mangled_type)
    {
        if (_type_map.size() == 0)
            init_type_map();
        std::map<std::string, std::string>::iterator iter =
                _type_map.find(std::string(mangled, 1));
        if (iter != _type_map.end()) {
            end_of_mangled_type = mangled + 1;
        }
        else {
            iter = _type_map.find(std::string(mangled, 2));
            if (iter !=_type_map.end()) {
                end_of_mangled_type = mangled + 2;
            }
            else {
                return NULL;
            }
...
std::map<std::string, std::string> __ocl_vector::_type_map;
char __ocl_vector::_buf[20];
int __ocl_vector::_size;
...
public:
    ...
    virtual size_t size() const
    {
        if (__cached_size_ == -1)
        {
            ...
            if (__right_ != 0)
                ...
                r += static_cast<size_t>(snprintf(0, 0, "%lu", (unsigned long)__size_));
            const_cast<long&>(__cached_size_) = static_cast<long>(r);
        }
        return static_cast<size_t>(__cached_size_);
    }
    ...
    virtual char* get_demangled_name(char* buf) const
    {
        ...
        *buf++ = '[';
        ...
        {
            int rs = sprintf(buf, "%lu", (unsigned long)__size_);
            buf += rs;
        }
        *buf++ = ']';
        return buf;
    ...
    virtual char* second_demangled_name(char* buf) const
    {
        ...
        *buf++ = '[';
        ...
        {
            int off = sprintf(buf, "%lu", (unsigned long)__size_);
            buf += off;
        }
        char* t = buf;
        buf = __left_->second_demangled_name(buf);
...
//                 ::= [U2A[N]] # Address space N, where N is valid for 0-5.
    ...
    else
    {
        for (; first != last; ++first)
        {
            ...
            case 'K':
                ...
                break;
            case 'U':
                if (!strncmp(first, "U1W", 3)) {
                  cv |= 64;
                  first += 2;
                } else if (!strncmp(first, "U1R", 3)) {
                  cv |= 32;
                  first += 2;
                } else if (!strncmp(first, "U1B", 3)) {
                  cv |= 128;
                  first += 2;
                } else if (!strncmp(first, "U2A0", 4)) {
                  cv |= 256;
                  first += 3;
                } else if (!strncmp(first, "U2A1", 4)) {
                  cv |= 512;
                  first += 3;
                } else if (!strncmp(first, "U2A2", 4)) {
                  cv |= 1024;
                  first += 3;
                } else if (!strncmp(first, "U2A3", 4)) {
                  cv |= 2048;
                  first += 3;
                } else if (!strncmp(first, "U2A4", 4)) {
                  cv |= 4096;
                  first += 3;
                } else if (!strncmp(first, "U2A5", 4)) {
                  cv |= 8192;
                  first += 3;
                }
                break;
...
__demangle_tree::__parse_builtin_type(const char* first, const char* last)
{
    if (first != last)
    {
        ...
        {
        case'u':
          if (!strncmp(first, "u3i1d", 4)) {
            if (__make<__image1d_t>()) {
              first += 4;
            }
          } else if (!strncmp(first, "u3i1b" , 4)) {
            if (__make<__image1d_buffer_t>()) {
              first += 4;
            }
          } else if (!strncmp(first, "u3i1a" , 4)) {
            if (__make<__image1d_array_t>()) {
              first += 4;
            }
          } else if (!strncmp(first, "u3i2d" , 4)) {
            if (__make<__image2d_t>()) {
              first += 4;
            }
          } else if (!strncmp(first, "u3i2b" , 4)) {
            if (__make<__image2d_buffer_t>()) {
              first += 4;
            }
          } else if (!strncmp(first, "u3i3d" , 4)) {
            if (__make<__image3d_t>()) {
              first += 4;
            }
          } else if (!strncmp(first, "u2ev" , 4)) {
            if (__make<__event_t>()) {
              first += 4;
            }
          } else if (!strncmp(first, "u2sa" , 4)) {
            if (__make<__sampler_t>()) {
              first += 4;
            }
          } else if (!strncmp(first, "u2sz" , 4)) {
            if (__make<__spir_size_t>()) {
              first += 4;
            }
          } else if (!strncmp(first, "u2pd" , 4)) {
            if (__make<__spir_ptrdiff_t>()) {
              first += 4;
            }
          } else if (!strncmp(first, "u2v", 3)
                  || !strncmp(first, "u3v", 3)) {
            const char* p = first + 3;
            int vector_size;
            if (first[1] == '2') {
                vector_size = p[0] - '0';
                p++;
            ...
            if (  vector_size == 2
               ...
               || vector_size == 16) {
                ...
                const char* element_type =
                        __ocl_vector::map_element_type(p, end);
                if (element_type != NULL) {
                    __make<__ocl_vector>(element_type, vector_size);
                    first = end;
                }
            }
          } else {
            __parse_source_name(first + 1, last);
          }
          break;
        case 'v':
            if (__make<__void>())
        ...
        case 'w':
            {
                ...
                {
                    if (__make<___wchar_t_literal>(first+2, t))
                        first = t+1;
                }
            }
            break;
...
__demangle_tree::__parse_static_cast_expr(const char* first, const char* last)
{
    if (last - first >= 3 && first[0] == 's' && first[1] == 'c')
    {
        ...
        if (t != first+2)
        {
            ...
            if (t1 != t)
            {
                if (__make<___static_cast>(type, __root_))
                    first = t1;
            }
        }
    }
...
#ifndef _MSC_VER
#define ALIGN_16 __attribute((aligned(16)))
#else
