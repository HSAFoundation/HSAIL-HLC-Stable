...
#if defined(AMD_OPENCL) || 1
void MCObjectFileInfo::InitBRIGDwarfMCObjectFileInfo(Triple T) {
  assert ( T.getArch() == Triple::hsail_64 || T.getArch() == Triple::hsail );
  BSSSection = 0; // no BSS

  TextSection = Ctx->getELFSection(".brigcode", ELF::SHT_NOBITS,
                                    0, SectionKind::getText());

  DataSection = Ctx->getELFSection(".brigdirectives", ELF::SHT_NOBITS,
  ...
  DwarfMacroInfoSection =
    Ctx->getELFSection(".debug_macinfo", ELF::SHT_PROGBITS, 0,
                       SectionKind::getMetadata());
  DwarfDebugInlineSection =
    Ctx->getELFSection(".debug_inlined", ELF::SHT_PROGBITS, 0,
                           SectionKind::getMetadata());
}
#endif
...
void MCObjectFileInfo::InitMCObjectFileInfo(StringRef TT, Reloc::Model relocm,
  ...
  if ((Arch == Triple::x86 || Arch == Triple::x86_64 ||
    ...
    Env = IsELF;
#if defined(AMD_OPENCL) || 1
    if (AMDOptions::isTargetHSAIL(TT)) {
      InitBRIGDwarfMCObjectFileInfo(T);
      return;
    }
#endif
