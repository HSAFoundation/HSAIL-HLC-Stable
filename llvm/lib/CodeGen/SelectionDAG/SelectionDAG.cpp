...
void SelectionDAG::ReplaceAllUsesWith(SDValue FromN, SDValue To) {
  ...
  AssignOrderingRecurs(To.getNode(), GetOrdering(From));
#endif
...
#if 1 || defined(AMD_OPENCL)
  // Propagate node ordering
  AssignOrderingRecurs(To, GetOrdering(From));
#endif
...
#if 1 || defined(AMD_OPENCL)
  unsigned Order = GetOrdering(From);
#endif
...
#if 1 || defined(AMD_OPENCL)
      // Propagate node ordering
      AssignOrderingRecurs(To[Use.getResNo()].getNode(), Order);
#endif
    } while (UI != UE && *UI == User);
...
#if 1 || defined(AMD_OPENCL)
  // Propagate node ordering
  AssignOrderingRecurs(To.getNode(), GetOrdering(From.getNode()));
#endif
...
#if 1 || defined(AMD_OPENCL)
      // Propagate node ordering
      AssignOrderingRecurs(To[i].getNode(), GetOrdering(From[i].getNode()));
#endif
...
#if 1 || defined(AMD_OPENCL)
/// GetOrdering - Get the order for the SDNode.
void SelectionDAG::AssignOrderingRecurs(const SDNode *SD, unsigned Order) {
  if (Order == 0) return; // avoid infinite recursion
  if (GetOrdering(SD) != 0) return; // Already has ordering.

  AssignOrdering(SD, Order);

  for (unsigned I = 0, E = SD->getNumOperands(); I != E; ++I)
    AssignOrderingRecurs(SD->getOperand(I).getNode(), Order);
}
#endif
...
SelectionDAG::SplitVector(const SDValue &N, const DebugLoc &DL, const EVT &LoVT,
                          const EVT &HiVT) {
  ...
  Lo = getNode(ISD::EXTRACT_SUBVECTOR, DL, LoVT, N,
               getConstant(0, TLI.getPointerTy()));
  Hi = getNode(ISD::EXTRACT_SUBVECTOR, DL, HiVT, N,
               getConstant(LoVT.getVectorNumElements(), TLI.getPointerTy()));
