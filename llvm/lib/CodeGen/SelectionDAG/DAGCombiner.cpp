...
SDValue DAGCombiner::visitANY_EXTEND(SDNode *N) {
  ...
  if (N0.getOpcode() == ISD::LOAD &&
    ...
    if (!LegalOperations || TLI.isLoadExtLegal(ExtType, MemVT)) {
      SDValue ExtLoad = DAG.getExtLoad(ExtType, N->getDebugLoc(),
...
SDValue DAGCombiner::visitSTORE(SDNode *N) {
  ...
  // This optimization to merge consecutive stores results in unaligned
  // stores and causes performance degradation in some of the
  // benchmarks. Due to this this optimization is currently disabled and
  // will be tracked through bug 9574.
#else
