...
static unsigned getMemoryScope(const Instruction &I) {
  MDNode *memScopeMD = I.getMetadata("mem.scope");
  return memScopeMD ?
      cast<ConstantInt>(memScopeMD->getOperand(0))->getZExtValue() : 0;
}
  ...
  case Intrinsic::dbg_declare: {
    ...
    if (isa<UndefValue>(Address) ||
      ...
      if (const AllocaInst *AI = dyn_cast<AllocaInst>(Address)) {
        MachineFunction& MF = DAG.getMachineFunction();
        MF.DIVToValueMap.insert(
          std::pair<const MDNode *, const Value *>(Variable, AI));
      }
#endif
