...
void SelectionDAGISel::CodeGenAndEmitDAG() {
  ...
  DEBUG(dbgs() << "Initial selection DAG: BB#" << BlockNumber
        << " '" << BlockName << "'\n"; CurDAG->dump());
  bool runDagCombine = true;
  if ((reinterpret_cast<const LLVMTargetMachine*>(&TM)->getTargetTriple() 
       == "amdil-pc-amdopencl"
       || reinterpret_cast<const LLVMTargetMachine*>(&TM)->getTargetTriple() 
          == "amdil64-pc-amdopencl")
  ...
  // Select target instructions for the DAG.
  {
    ...
    while (ISelPosition != CurDAG->allnodes_begin()) {
      ...
      if (ResNode) {
        ...
        CurDAG->AssignOrderingRecurs(ResNode, CurDAG->GetOrdering(Node));
#else
...
#if 1 || defined(AMD_OPENCL)
  RegisterScheduler::FunctionPassCtor CurCtor = ISHeuristic;

  // Restore ISHeuristic to the default
  ISHeuristic.setInitialValue(&createDefaultScheduler);

  return CurCtor(this, OptLevel);
