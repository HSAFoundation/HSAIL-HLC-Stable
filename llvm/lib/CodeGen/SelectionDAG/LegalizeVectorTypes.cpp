...
//  case ISD::FP_ROUND:          SplitVecRes_FP_ROUND(N, Lo, Hi); break;
//#endif
...
bool DAGTypeLegalizer::ScalarizeVectorOperand(SDNode *N, unsigned OpNo) {
  ...
  DAG.AssignOrderingRecurs(Res.getNode(), DAG.GetOrdering(N));
#endif
...
#if AMD_OPENCL_CODE || 1
void DAGTypeLegalizer::SplitVecRes_FP_ROUND(SDNode *N, SDValue &Lo,
					    SDValue &Hi) {
  DebugLoc dl = N->getDebugLoc();
  GetSplitVector(N->getOperand(0), Lo, Hi);
  ...
  EVT EltType = N->getValueType(0).getVectorElementType();
  EVT NewVecVT = EVT::getVectorVT(*DAG.getContext(), EltType, NumElts);
  

  Lo = DAG.getNode(ISD::FP_ROUND, dl, NewVecVT, Lo, N->getOperand(1));
  Hi = DAG.getNode(ISD::FP_ROUND, dl, NewVecVT, Hi, N->getOperand(1));
}  
#endif
