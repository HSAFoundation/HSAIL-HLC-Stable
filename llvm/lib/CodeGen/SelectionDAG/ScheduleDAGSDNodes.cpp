...
#if defined(AMD_OPENCL) || 1
void ScheduleDAGSDNodes::ClusterNeighboringLdSt(SDNode *Node)
{
  // Here we expect that processes for clustering loads and stores 
  // are essentially same. 
  // And with goal of avoiding copy and paste internal `ClusterNeighboringLoads`
  // code we use only one function for both load and store nodes.
  // WARNING! This can stop working if `ClusterNeighboringLoads` changes it
  // behavior in such way that it can no longer transparently handle stores.
