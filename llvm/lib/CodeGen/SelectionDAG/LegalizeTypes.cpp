...
void DAGTypeLegalizer::SetPromotedInteger(SDValue Op, SDValue Result) {
  ...
  DAG.AssignOrderingRecurs(Result.getNode(), DAG.GetOrdering(Op.getNode()));
#else
...
void DAGTypeLegalizer::GetExpandedInteger(SDValue Op, SDValue &Lo,
  ...
  // Propagate ordering
#if 1 || defined(AMD_OPENCL)
  DAG.AssignOrderingRecurs(Lo.getNode(), DAG.GetOrdering(Op.getNode()));
  DAG.AssignOrderingRecurs(Hi.getNode(), DAG.GetOrdering(Op.getNode()));
#else
...
bool DAGTypeLegalizer::CustomLowerNode(SDNode *N, EVT VT, bool LegalizeResult) {
  ...
  for (unsigned i = 0, e = Results.size(); i != e; ++i) {
    ...
    // Propagate node ordering
#if 1 || defined(AMD_OPENCL)
    DAG.AssignOrderingRecurs(Results[i].getNode(), DAG.GetOrdering(N));
#else
