...
unsigned RegReductionPQBase::getNodePriority(const SUnit *SU) const {
  ...
  if (SU->getNode() && TII->shouldScheduleWithNormalPriority(SU->getNode())) {
...
#if defined(AMD_OPENCL) || 1
  const char* pMaxReoderWindow = std::getenv("AMD_DEBUG_SCHEDULER_MAX_REORDER");
  const char* pSchedCriticalPath = std::getenv("AMD_DEBUG_SCHEDULER_CRIT_PATH");
  if (pMaxReoderWindow) {
    static bool reported = false;
    MaxReorderWindow = std::atoi(pMaxReoderWindow);
    if (!reported) {
      printf("Overriding scheduler max reorder window with %s\n", pMaxReoderWindow);
      reported = true;
    }
  }
  if (pSchedCriticalPath) {
    static bool reported = false;
    DisableSchedCriticalPath = (*pSchedCriticalPath == '1') ? false : true;
    if (!reported) {
      printf("Overriding -disable-sched-critical-path with %d\n",
             (int)DisableSchedCriticalPath);
      reported = true;
    }
  }
#endif // AMD_OPENCL
