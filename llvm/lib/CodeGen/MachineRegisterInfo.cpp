...
/// replaceDbgVAlueRegWidth - Replaces all instances of FromReg with ToReg in
/// the machine function. This is like llvm-level X->replaceAllUsesWith(Y),
/// except that it also changes any definitions of the register as well.
/// This function only replaces the register for debug values and moves the
/// debug value to exist right after the instruction that defines the register
/// that the debug information will reference.
void MachineRegisterInfo::replaceDbgValueRegWith(unsigned FromReg, unsigned ToReg) {
  assert(FromReg != ToReg && "Cannot update debug information with itself");

  for (reg_iterator I = reg_begin(FromReg), E = reg_end(); I != E; ) {
    MachineOperand &O = I.getOperand();
    MachineInstr *pMI = O.getParent();
    // If this operand is part of a debug value instruction, then
    // the debug value instruction needs to be updated to exist
    // right after the original def instruction.
    if (pMI->isDebugValue()) {
      MachineInstr *sMI = getRegUseDefListHead(ToReg)->getParent();
      pMI->removeFromParent();
      // Make sure the debug loc is updated correctly.
      sMI->setDebugLoc(pMI->getDebugLoc());
      O.setReg(ToReg);
      MachineBasicBlock::iterator isMI = sMI;
      sMI->getParent()->insertAfter(isMI, pMI);
    }
    ++I;
  }
}
