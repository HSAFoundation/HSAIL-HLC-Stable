...
static bool EnableRetToEnd = false;
static cl::opt<cl::boolOrDefault> FlagEnableRetToEnd("enable-ret-to-end",
  cl::desc("Make block with return instruction to be the last block in the function"),
  cl::init(cl::BOU_UNSET), cl::Hidden);

static bool DoNotTailMergeAcrossLoop = false;
static cl::opt<cl::boolOrDefault> FlagDoNotTailMergeAcrossLoop(
  "do-not-tail-merge-across-loop",
  cl::desc("Do not tail merge blocks across loop boundaries "
           "as it could introduce multiple entry into loops"),
  cl::init(cl::BOU_UNSET), cl::Hidden);
#endif
...
#if defined(AMD_OPENCL) || 1
  BranchFolder Folder(PassConfig->getEnableTailMerge(),
                      /*CommonHoist=*/true,
                      PassConfig->getEnableRetToEnd(),
                      PassConfig->getDisableTailMergeAcrossLoop());
...
#if defined(AMD_OPENCL) || 1
  switch (FlagEnableRetToEnd) {
  case cl::BOU_UNSET: EnableRetToEnd = defaultEnableRetToEnd; break;
  case cl::BOU_TRUE: EnableRetToEnd = true; break;
  case cl::BOU_FALSE: EnableRetToEnd = false; break;
  }

  switch (FlagDoNotTailMergeAcrossLoop) {
  case cl::BOU_UNSET:
    DoNotTailMergeAcrossLoop = defaultDoNotTailMergeAcrossLoop; break;
  case cl::BOU_TRUE: DoNotTailMergeAcrossLoop = true; break;
  case cl::BOU_FALSE: DoNotTailMergeAcrossLoop = false; break;
  }
#endif
...
bool BranchFolder::OptimizeFunction(MachineFunction &MF,
  ...
  if (EnableRetToEnd)
  {
    // Move return blocks to the end of function in order to simplify finalizer work
    MachineBasicBlock *EBB = --MF.end();
    for (MachineFunction::iterator I = MF.begin(), E = MF.end(); I != E; ++I) {
      MachineBasicBlock *MBB = I;

      if (MBB != &MF.back() &&
          MBB->getLastNonDebugInstr()->getDesc().isReturn()) {

        // If previous block has fall through to MBB and
        // we don't have something like 
        //   cbr MBB
        //   br  ...
        // then insert new branch from PrevBB to MBB
        if (I != MF.begin()) {
          MachineBasicBlock *PrevBB = prior(I);

          if (PrevBB->canFallThrough() && 
              !PrevBB->getLastNonDebugInstr()->getDesc().isUnconditionalBranch()) {
            MachineBasicBlock::iterator mi = MBB->begin();
            TII->InsertBranch(*PrevBB, MBB, 0, SmallVector<MachineOperand, 0>(),
              MBB->findDebugLoc(mi));
          }
        }
        ...
        // Move block with return
        MBB->moveAfter(--MF.end());

        // Remove fallthorugh branches.
        // We can not call OptimizeBlock because it will result
        // in more optimal placement of MBB.
        // Call AnalyzeBranch because it is very probable that it will
        // do fallthrough removal.
        MachineBasicBlock *TBB = 0, *FBB = 0;

        SmallVector<MachineOperand, 0> Cond;
        TII->AnalyzeBranch(*prior(--MF.end()), TBB, FBB, Cond, true);
      }
    }
  }
#endif // AMD_OPENCL
...
static void FixTail(MachineBasicBlock *CurMBB, MachineBasicBlock *SuccBB,
  const TargetInstrInfo *TII, bool IsCondReversed) {
#else
static void FixTail(MachineBasicBlock *CurMBB, MachineBasicBlock *SuccBB,
  const TargetInstrInfo *TII) {
#endif // AMD_OPENCL
...
#if defined(AMD_OPENCL) || 1
  // We need to restore bb tail exactly as it was before convertion
  // to canonical form in order not to lose any previous branch folding
  // optimizations.
  if (!TII->AnalyzeBranch(*CurMBB, TBB, FBB, Cond, true)) {
    if (IsCondReversed) {
      if (!TII->ReverseBranchCondition(Cond)) {
        TII->RemoveBranch(*CurMBB);
        TII->InsertBranch(*CurMBB, SuccBB, TBB, Cond, dl);
        return;
      }
    }
    ...
    if (I != MF->end() && TBB == NextBB && !Cond.empty() && !FBB) {
      if (!TII->ReverseBranchCondition(Cond)) {
        TII->RemoveBranch(*CurMBB);
...
#if defined(AMD_OPENCL) || 1
      FixTail(CurMBB, SuccBB, TII, CurMPIter->getIsCondReversed());
#else
...
namespace
{
  // Helper for finding calls with std::find_if
  struct _predicateIsCall
  {
    bool operator()(MachineInstr &i)  { return i.getDesc().isCall(); }
  };
}
#endif
...
#if defined(AMD_OPENCL) || 1
    // Do not merge blocks inside loops with blocks outside loops.
    // Because it will introduce multiple loop entries
    // which is bad case for finalyzer
    if (DoNotTailMergeAcrossLoop && MLI)
    {
      MachineLoop *commonLoop = MLI->getLoopFor(SameTails[0].getBlock());
      bool is_blocks_in_same_loop = true;

      for (unsigned i = 1, e = SameTails.size(); i != e; ++i) {
        if (MLI->getLoopFor(SameTails[i].getBlock()) != commonLoop) {
          is_blocks_in_same_loop = false;
          break;
        }
      }

      if (!is_blocks_in_same_loop) {
        RemoveBlocksWithHash(CurHash, SuccBB, PredBB);
        continue;
      }
    }

    // If at least one of blocks contains call instruction - give up,
    // we don't want to break argument scopes
    bool one_of_blocks_has_call = false;

    for (unsigned i = 1, e = SameTails.size(); i != e; ++i) {
      MachineBasicBlock *mbb = SameTails[i].getBlock();

      if (std::find_if(mbb->begin(), mbb->end(), ::_predicateIsCall()) !=
            mbb->end()) {
        one_of_blocks_has_call = true;
        break;
      }
    }

    if (one_of_blocks_has_call)
    {
      RemoveBlocksWithHash(CurHash, SuccBB, PredBB);
      continue;
    }
...
bool BranchFolder::TailMergeBlocks(MachineFunction &MF) {
  ...
  for (MachineFunction::iterator I = MF.begin(), E = MF.end();
    ...
    if (I->succ_empty())
#if defined(AMD_OPENCL) || 1
      MergePotentials.push_back(MergePotentialsElt(HashEndOfMBB(I), I, false));
#else
  ...
  for (MachineFunction::iterator I = llvm::next(MF.begin()), E = MF.end();
...
#if defined(AMD_OPENCL) || 1
        MergePotentials.push_back(MergePotentialsElt(HashEndOfMBB(PBB), *P, IsCondReversed));
#else
    ...
    if (MergePotentials.size() == 1 &&
      ...
      FixTail(MergePotentials.begin()->getBlock(),IBB,TII,
		      MergePotentials.begin()->getIsCondReversed());
#else
      FixTail(MergePotentials.begin()->getBlock(), IBB, TII);
#endif
  }
...
#if defined(AMD_OPENCL) || 1
        // However we still can get additional fallthrough if succ already have
        // a block that can fallthrough into it and this block can fallthrough
        // to MBB
        MachineBasicBlock *SuccPrevTBB = 0, *SuccPrevFBB = 0;
        SmallVector<MachineOperand, 4> SuccPrevCond;
        bool SuccPrevUnAnalyzable = TII->AnalyzeBranch(
          *SuccPrev, SuccPrevTBB, SuccPrevFBB, SuccPrevCond, true);

        if (SuccBB != MBB && &*SuccPrev != MBB && !CurUnAnalyzable &&
            (!SuccPrev->canFallThrough() ||
             (!SuccPrevUnAnalyzable &&
              (SuccPrevTBB == MBB || SuccPrevFBB == MBB))) &&
            !SuccBB->isLandingPad()) {
          // If SuccPrev fall through to SuccBB but it doesn't contain
          // unconditional branch at the end
          if (SuccPrev->canFallThrough() && !SuccPrevFBB)
          {
            SuccPrevCond.clear();
            TII->InsertBranch(*SuccPrev, SuccBB, 0, SuccPrevCond, DebugLoc());
          }
