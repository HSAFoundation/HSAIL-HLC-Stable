...
static void GetLinearExpression(Value *V, APInt &Offset,
                                SmallVectorImpl<ScaledVariable>& ScaledVars,
                                ExtensionKind &Extension,
  ...
  if (Depth == 6) {
    Offset = 0;
    ScaledVariable Entry = {V, Extension, APInt(Offset.getBitWidth(), 1)};
    ScaledVars.push_back(Entry);
    return;
  }
  ...
  if (BinaryOperator *BOp = dyn_cast<BinaryOperator>(V)) {
    if (ConstantInt *RHSC = dyn_cast<ConstantInt>(BOp->getOperand(1))) {
      ...
      case Instruction::Add:
        GetLinearExpression(BOp->getOperand(0), Offset, ScaledVars, Extension,
                            DL, Depth+1);
      ...
      case Instruction::Mul:
      {
        GetLinearExpression(BOp->getOperand(0), Offset, ScaledVars, Extension,
                            DL, Depth+1);
        Offset *= RHSC->getValue();
        SmallVectorImpl<ScaledVariable>::iterator it = ScaledVars.begin();
        SmallVectorImpl<ScaledVariable>::iterator end = ScaledVars.end();
        for (; it != end; ++it) {
          (*it).Scale *= RHSC->getValue();
        }
        return;
      }
      case Instruction::Shl:
      {
        ...
        SmallVectorImpl<ScaledVariable>::iterator end = ScaledVars.end();
        for (; it != end; ++it) {
          (*it).Scale <<= RHSC->getValue().getLimitedValue();
        }
        return;
      }
      }
      ...
      GetLinearExpression(BOp->getOperand(0), Offset, ScaledVars, Extension,
                          DL, Depth+1);
      SmallVector<ScaledVariable,2> ScaledVars2;
      APInt Offset2(Offset.getBitWidth(), 0);
      GetLinearExpression(BOp->getOperand(1), Offset2, ScaledVars2, Extension,
                          DL, Depth+1);
      ScaledVars.append(ScaledVars2.begin(), ScaledVars2.end());
      Offset += Offset2;
      return;
    }
  }
  ...
  if ((isa<SExtInst>(V) && Extension != EK_ZeroExt) ||
    ...
    assert(ScaledVars.empty() && "unexpected vars");
    Offset = Offset.trunc(SmallWidth);
    Extension = isa<SExtInst>(V) ? EK_SignExt : EK_ZeroExt;

    GetLinearExpression(CastOp, Offset, ScaledVars, Extension, DL, Depth+1);
    SmallVectorImpl<ScaledVariable>::iterator it = ScaledVars.begin();
    SmallVectorImpl<ScaledVariable>::iterator end = ScaledVars.end();
    for (; it != end; ++it) {
      (*it).Scale = (*it).Scale.zext(OldWidth);
    }
...
DecomposeGEPExpression(const Value *V, int64_t &BaseOffs,
  ...
  do {
    ...
    for (User::const_op_iterator I = GEPOp->op_begin()+1,
      ...
      if (DL->getPointerSizeInBits() > Width)
        Extension = EK_SignExt;
      
      // Use GetLinearExpression to decompose the index into a C1*V+C2 form.
#if 1 || defined (AMD_OPENCL_MEMCOMBINE)
      APInt IndexOffset(Width, 0);
      SmallVector<ScaledVariable,2> ScaledVars;
      GetLinearExpression(Index, IndexOffset, ScaledVars, Extension,
                          *DL, 0);
      ...
      SmallVectorImpl<ScaledVariable>::iterator end = ScaledVars.end();
      for (; it != end; ++it) {
        uint64_t thisScale = Scale * (*it).Scale.getSExtValue();
        ...
        for (unsigned i = 0, e = VarIndices.size(); i != e; ++i) {
          if (VarIndices[i].V == (*it).V &&
              VarIndices[i].Extension == (*it).Extension) {
            thisScale += VarIndices[i].Scale;
        ...
        // pointer size.
        if (unsigned ShiftBits = 64-DL->getPointerSizeInBits()) {
          thisScale <<= ShiftBits;
          thisScale = (int64_t)thisScale >> ShiftBits;
        }

        if (thisScale) {
          VariableGEPIndex Entry = {(*it).V, (*it).Extension,
                                   static_cast<int64_t>(thisScale)};
