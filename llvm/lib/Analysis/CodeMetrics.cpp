...
#if defined(AMD_OPENCL) || 1
  // These AMDIL intrinsics for the most part turn into an instruction.
  if (Name.startswith("__amdil"))
	 return true;
  if (Name.startswith("__hsail"))
	 return true;
#endif
  return false;
}
...
void CodeMetrics::analyzeBasicBlock(const BasicBlock *BB,
  ...
  for (BasicBlock::const_iterator II = BB->begin(), E = BB->end();
    ...
    if (isa<ExtractElementInst>(II) || II->getType()->isVectorTy())
      ++NumVectorInsts;
#if defined(AMD_OPENCL) || 1
    // For a scalar language like HSAIL each vector arithmetic operation
    // produces several instructions.
    // At the same time vector loads, GEP, extractelement, insertelement
    // or shuffle can be done in a single operation.
    // Therefore only multiply by vector size standard binary operations.
    // TODO_HSA: Use TargetTransformInfo::getUserCost once it will be avaliable
    //           Bug 9479
    if (AMDOptions::isTargetHSAIL(II) && 
        II->getType()->isVectorTy() && isa<BinaryOperator>(II)) {
      NumInsts += II->getType()->getVectorNumElements();
    } else
#endif
    ++NumInsts;
  }
