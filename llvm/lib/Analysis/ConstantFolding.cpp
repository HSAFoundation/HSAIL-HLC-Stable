...
#include "AMDConstantFolding.inc"
#endif // AMD_OPENCL
...
#if defined(AMD_OPENCL) || 1
  if (AMDCanConstantFoldCallTo(Name))
    return true;
#endif
...
llvm::ConstantFoldCall(Function *F, ArrayRef<Constant *> Operands,
  ...
  StringRef Name = F->getName();
#if defined(AMD_OPENCL) || 1
  if (Constant *C = AMDConstantFoldCall(F, Name, Operands))
    return C;
#endif
    ...
    if (ConstantFP *Op = dyn_cast<ConstantFP>(Operands[0])) {
...
#if defined(AMD_OPENCL) || 1
        if (Name == "exp10" && TLI->has(LibFunc::exp10))
          return ConstantFoldBinaryFP(pow, 10.0, V, Ty);
#endif
        break;
      case 'f':
...
#if defined(AMD_OPENCL) || 1
        else if (Name == "log2" && V > 0 && TLI->has(LibFunc::log2))
      ...
      case 'r':
        if (Name == "round" && TLI->has(LibFunc::round))
          return ConstantFoldFP(floor, V + 0.5, Ty);
        break;
#endif
      case 's':
