...
static cl::opt<unsigned>
BinomialCoefficientLimitBitwidth("binomial-coefficient-limit-bitwidth", cl::Hidden,
								 cl::desc("Binomial coefficient can sometimes generate wide "
								          "arithemtic that is limited to the number of bits "
...
#if 1 || defined(AMD_OPENCL)
  // In some cases and on some hardware, the number of bits that
  // can be generated is unreasonable. In this situation we check
  // to make sure that the number of resulting bits is <= to some
  // user defined limit. The default value of 2048 should be large
  // enough that not many people in practice should hit it.  This
  // option might not produce correct results in certain cases where
  // overflow exists.
  // FIXME: remove this if LLVM proper implements a better solution
  if (CalculationBits > BinomialCoefficientLimitBitwidth) {
	  CalculationBits = BinomialCoefficientLimitBitwidth;
  }
