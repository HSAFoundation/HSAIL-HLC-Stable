...
X86TargetLowering::X86TargetLowering(X86TargetMachine &TM)
  ...
  if (!Subtarget->is64Bit() && Subtarget->isTargetAMDOpenCL()) {
    // These libcalls are handled specificly by AMD OpenCL
  ...
  // FIXME: Bug 9250: Library should provide an implementation of these.
  // Those functions must have been generated by the compiler, and
  // should always be expanded into loads/stores.
  maxStoresPerMemset  = 4096; // For @llvm.memset -> sequence of stores
  maxStoresPerMemcpy  = 4096; // For @llvm.memcpy -> sequence of stores
  maxStoresPerMemmove = 4096; // For @llvm.memmove -> sequence of stores
#endif
}
...
#if AMD_OPENCL_CODE || 1
  // Temporarily disable because it is unsafe.
  // See http://llvm.org/bugs/show_bug.cgi?id=10114
  return SDValue();
#endif
...
SDValue X86TargetLowering::LowerUINT_TO_FP(SDValue Op,
  ...
  if ((Subtarget->isTargetWindows() || Subtarget->isTargetMingw()) &&
    ...
    SDValue hi = DAG.getNode(ISD::TRUNCATE, dl, MVT::i32, hi_shift);
    SDValue bsrhi = DAG.getNode(X86ISD::BSR, dl, MVT::i32, hi, DAG.getConstant(63, MVT::i32));
    SDValue bsrhix = DAG.getNode(ISD::XOR, dl, MVT::i32, DAG.getConstant(31, MVT::i32), bsrhi);
    SDValue lo = DAG.getNode(ISD::TRUNCATE, dl, MVT::i32, u);
    SDValue bsrlo = DAG.getNode(X86ISD::BSR, dl, MVT::i32, lo, DAG.getConstant(63, MVT::i32));
    SDValue bsrlox = DAG.getNode(ISD::XOR, dl, MVT::i32, DAG.getConstant(31, MVT::i32), bsrlo);
    SDValue loresult = DAG.getNode(ISD::ADD, dl, MVT::i32, bsrlox, DAG.getConstant(32, MVT::i32));
    SDValue hinotzero = DAG.getSetCC(dl, getSetCCResultType(*DAG.getContext(), MVT::i32), hi,
                                     DAG.getConstant(0, MVT::i32), ISD::SETNE);
    SDValue clzresult = DAG.getNode(ISD::SELECT, dl, MVT::i32, hinotzero, bsrhix, loresult);
    SDValue unotzero = DAG.getSetCC(dl, getSetCCResultType(*DAG.getContext(), MVT::i64), u,
                                    DAG.getConstant(0, MVT::i64), ISD::SETNE);
    SDValue unotzeroe = DAG.getNode(ISD::SUB, dl, MVT::i32, DAG.getConstant(190, MVT::i32), clzresult);
    SDValue e = DAG.getNode(ISD::SELECT, dl, MVT::i32, unotzero, unotzeroe, DAG.getConstant(0, MVT::i32));
    SDValue ushl = DAG.getNode(ISD::SHL, dl, MVT::i64, u, clzresult);
    SDValue newu = DAG.getNode(ISD::AND, dl, MVT::i64, ushl, DAG.getConstant((uint64_t)0x7fffffffffffffffULL, MVT::i64));
    SDValue t = DAG.getNode(ISD::AND, dl, MVT::i64, newu, DAG.getConstant((uint64_t)0xffffffffffULL, MVT::i64));
    SDValue shle = DAG.getNode(ISD::SHL, dl, MVT::i32, e, DAG.getConstant(23, MVT::i32));
    SDValue shrnewu = DAG.getNode(ISD::SRL, dl, MVT::i64, newu, DAG.getConstant(40, MVT::i64));
    SDValue shrnewutrunc = DAG.getNode(ISD::TRUNCATE, dl, MVT::i32, shrnewu);
    SDValue v = DAG.getNode(ISD::OR, dl, MVT::i32, shle, shrnewutrunc);
    SDValue t8 = DAG.getSetCC(dl, getSetCCResultType(*DAG.getContext(), MVT::i64), t,
                              DAG.getConstant((uint64_t)0x8000000000ULL, MVT::i64), ISD::SETEQ);
    SDValue tbig = DAG.getSetCC(dl, getSetCCResultType(*DAG.getContext(), MVT::i64), t,
                                DAG.getConstant((uint64_t)0x8000000000ULL, MVT::i64), ISD::SETGT);
    SDValue vlowbit = DAG.getNode(ISD::AND, dl, MVT::i32, v, DAG.getConstant(1, MVT::i32));
    SDValue tsub = DAG.getNode(ISD::SELECT, dl, MVT::i32, t8, vlowbit, DAG.getConstant(0, MVT::i32));
    SDValue r = DAG.getNode(ISD::SELECT, dl, MVT::i32, tbig, DAG.getConstant(1, MVT::i32), tsub);
    SDValue intresult = DAG.getNode(ISD::ADD, dl, MVT::i32, v, r);
