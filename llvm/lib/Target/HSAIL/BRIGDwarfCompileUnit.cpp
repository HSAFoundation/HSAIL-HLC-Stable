...
namespace llvm {
      ...
      if (Location.getReg() == TRI->getFrameRegister(MF)) {
        ...
        if (m_brigAP->getPrivateStackOffset(&stackOffset)
          ...
          do {
            ...
            addUInt(Block, 0, dwarf::DW_FORM_data1, Byte);
          } while (offs);
          addBlock(Die, dwarf::DW_AT_location, 0, Block);
          return;
        }
        // otherwise let DwarfCompileUnit handle this address
      }
    }
    CompileUnit::addVariableAddress(DV, Die, Location);
  }
  ...
  void BRIGDwarfCompileUnit::addLabel(DIE *Die, unsigned Attribute, 
    ...
    this->CompileUnit::addLabel(Die, Attribute, Form, Label);
  }

  void BRIGDwarfCompileUnit::addDelta(DIE *Die, unsigned Attribute, unsigned Form,
                          const MCSymbol *Hi, const MCSymbol *Lo) {
    this->CompileUnit::addDelta(Die, Attribute, Form, Hi, Lo);
  }
  ...
  void BRIGDwarfCompileUnit::createGlobalVariableDIE(const MDNode *N) {
    ...
    this->CompileUnit::createGlobalVariableDIE(N);
  }
    ...
    if (Offset != 0) {
      ...
      for (unsigned int i = 0, e = offsets.size(); i != e; i++) {
        ...
        if (size != 1) {
          ...
          assert(size/8 < 128); 
          addUInt(Block, 0, dwarf::DW_FORM_data1, dwarf::DW_OP_piece);
          ...
          addUInt(Block, 0, dwarf::DW_FORM_data1, size/8);
        }
        else {
          /* describe 1 bit piece of variable that is 1 bit next to the previous part */
          addUInt(Block, 0, dwarf::DW_FORM_data1, dwarf::DW_OP_bit_piece);
          /* 1 bit piece */
          addUInt(Block, 0, dwarf::DW_FORM_data1, 1);
          /* 1 bit from previous part, or 0 if it's the first element in bit vector */
          addUInt(Block, 0, dwarf::DW_FORM_data1, i == 0 ? 0 : 1);
        }
      }
    }
