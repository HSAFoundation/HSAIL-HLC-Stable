...
$(intermediates)/%GenSubtarget.inc: TBLGEN_LOCAL_MODULE := $(LOCAL_MODULE)
$(intermediates)/%GenSubtarget.inc: $(tblgen_source_dir)/%.td \
                                       $(tblgen_td_deps) | $(TBLGEN)
	$(call transform-td-to-out,subtarget)
endif

ifneq ($(filter %GenBrigWriter.inc,$(tblgen_gen_tables)),)
$(intermediates)/%GenBrigWriter.inc: TBLGEN_LOCAL_MODULE := $(LOCAL_MODULE)
$(intermediates)/%GenBrigWriter.inc: $(tblgen_source_dir)/%.td \
                                       $(tblgen_td_deps) | $(TBLGEN)
	$(call transform-td-to-out,brig-writer)
endif

ifneq ($(filter %GenVecMap.inc,$(tblgen_gen_tables)),)
$(intermediates)/%GenVecMap.inc: TBLGEN_LOCAL_MODULE := $(LOCAL_MODULE)
$(intermediates)/%GenVecMap.inc: $(tblgen_source_dir)/%.td \
                                       $(tblgen_td_deps) | $(TBLGEN)
	$(call transform-td-to-out,hsail-vec-map)
endif
#end
