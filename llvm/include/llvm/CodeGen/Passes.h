...
protected:
  ...
  // Do not tail merge blocks across loop boundaries
  // as it could introduce multiple entry into loops
  // Default setting for -do-not-tail-merge-across-loop
  bool DisableTailMergeAcrossLoop;

  // Make block with return instruction to be the last block in the function
  bool EnableRetToEnd;

  // Check for maximum number of registers LSR allowed to recalculate for each
  // formula.
  bool LsrCheckRegRecalc;

  // Ignore addrec cost in lsr solution comparison.
  bool LsrIgnoreAddRecCost;

  // Do not pick lose LSR solution on cost tie.
  bool LsrCheckLoserOnTie;
#endif
...
#if defined(AMD_OPENCL) || 1
  bool getDisableTailMergeAcrossLoop() const {
    return DisableTailMergeAcrossLoop;
  }
  void setDisableTailMergeAcrossLoop(bool Enable) {
    setOpt(DisableTailMergeAcrossLoop, Enable);
  }

  bool getEnableRetToEnd() const { return EnableRetToEnd; }
  void setEnableRetToEnd(bool Enable) { setOpt(EnableRetToEnd, Enable); }

  bool getLsrCheckRegRecalc() const { return LsrCheckRegRecalc; }
  void setLsrCheckRegRecalc(bool Enable) {
    setOpt(LsrCheckRegRecalc, Enable);
  }

  bool getLsrIgnoreAddRecCost() const { return LsrIgnoreAddRecCost; }
  void setLsrIgnoreAddRecCost(bool Enable) {
    setOpt(LsrIgnoreAddRecCost, Enable);
  }

  bool getLsrCheckLoserOnTie() const { return LsrCheckLoserOnTie; }
  void setLsrCheckLoserOnTie(bool Enable) {
    setOpt(LsrCheckLoserOnTie, Enable);
  }
#endif
...
namespace llvm {
  ...
  /// MachineInstCount - Counts the number of each type of instruction that is
  /// emitted.
  extern char &MachineInstrCountID;
  FunctionPass *createMachineInstrCountPass();
