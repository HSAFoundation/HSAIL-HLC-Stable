...
public:
#if defined(AMD_OPENCL) || 1
  Loop() : PragmaInfo(NULL) {}

  ~Loop() {
    if (PragmaInfo) {
      PragmaInfo->setMetadata(this);
      delete PragmaInfo;
    }
  }

  void setPragmaInfo(LoopPragmaInfo *PI) { PragmaInfo = PI; }
  LoopPragmaInfo* getPragmaInfo() const { return PragmaInfo; }
#else
  Loop() {}
#endif
