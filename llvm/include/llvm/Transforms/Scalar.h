...
FunctionPass *createInstructionCombiningPass(bool EnableUnsafeMathFP);
#endif
//===----------------------------------------------------------------------===//
//
...
//===----------------------------------------------------------------------===//
//
// MemCombine - This pass tried to vectorize memory accesses
//
BasicBlockPass *createAMDMemCombinePass(unsigned maxVecGen, unsigned minAlign);
//===----------------------------------------------------------------------===//
//
// DataLayout - This pass tried to change data layout to avoid bank conflict
//
ModulePass *createAMDScalarReplArrayElemPass(unsigned bankWidth,
...
//===----------------------------------------------------------------------===//
//
// VectorCoarsening - This pass tries to widen vector element types
//
FunctionPass *createVectorCoarseningPass(unsigned MinVecLen=0);
#endif
...
#if 1 || defined(AMD_OPENCL)
FunctionPass *createAMDDbgmovePass();
FunctionPass *createAMDSimplifyLibCallsPass(bool UnsafeMath = false);
FunctionPass *createAMDUseNativeCallsPass(const char*);
FunctionPass *createAMDPeepholeOpt(unsigned OptLevel);

ModulePass *createAMDEDGToIA64TranslatorPass(bool SetSPIRCC);
ModulePass *createAMDLowerAtomicsPass();
ModulePass *createAMDLowerPipeBuiltinsPass();
