 ...
 /// dag combiner.
  ...
  getCondCodeAction(ISD::CondCode CC, EVT VT) const {
    assert((unsigned)CC < array_lengthof(CondCodeActions) &&
           ((unsigned)VT.getSimpleVT().SimpleTy >> 4) < array_lengthof(CondCodeActions[0]) &&
           "Table isn't big enough!");
    uint32_t SimpleTy = VT.getSimpleVT().SimpleTy;
    uint32_t Shift = 2 * (SimpleTy & 0xF);
    uint32_t Value = CondCodeActions[CC][SimpleTy >> 4];
  ...
  // This method has been made virtual
  // to handle image/sampler object pointers in a specific way
  // for the HSAIL backend.
  virtual
#endif
...
protected:
  ...
  void setCondCodeAction(ISD::CondCode CC, MVT VT,
    ...
    CondCodeActions[(unsigned)CC][VT.SimpleTy >> 4] &= ~((uint32_t)3 << Shift);
    CondCodeActions[(unsigned)CC][VT.SimpleTy >> 4] |= (uint32_t)Action << Shift;
  }
