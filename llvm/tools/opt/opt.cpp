...
// Since the disble-inling flag is used with compiler library it is defined 
// in amdOptOptions.cpp
//
static cl::opt<bool>
...
static cl::opt<bool>
OptLevelO0("O0",
           cl::desc("Optimization level 0. Minimum optimization for correct compilation"));
#endif
...
static void AddOptimizationPasses(Module *M, PassManagerBase &MPM,FunctionPassManager &FPM,
...
#if 1 || defined(AMD_OPENCL)
  if (AMDAddOptimizationPasses(M, MPM, FPM, OptLevel)) {
    return;
  }
#endif
...
int main(int argc, char **argv) {
...
#if defined(AMD_OPENCL) || 1
  initializeHSAILFixSpecialIntrinsicsPass(Registry);
  initializeAMDLowerAtomicsPass(Registry);
#endif
  ...
  AMDSetContextHook(&Context, &AmdHook);
#endif
  ...
  for (unsigned i = 0; i < PassList.size(); ++i) {
    ...
    // HSA_TODO: What happens at -O0?

    if (OptLevelO1 && OptLevelO1.getPosition() < PassList.getPosition(i)) {
      AddOptimizationPasses(M.get(), Passes, *FPasses, 1, 0);
      OptLevelO1 = false;
    }

    if (OptLevelO2 && OptLevelO2.getPosition() < PassList.getPosition(i)) {
      AddOptimizationPasses(M.get(), Passes, *FPasses, 2, 0);
      OptLevelO2 = false;
    }

    if (OptLevelOs && OptLevelOs.getPosition() < PassList.getPosition(i)) {
      AddOptimizationPasses(M.get(), Passes, *FPasses, 2, 1);
      OptLevelOs = false;
    }

    if (OptLevelOz && OptLevelOz.getPosition() < PassList.getPosition(i)) {
      AddOptimizationPasses(M.get(), Passes, *FPasses, 2, 2);
      OptLevelOz = false;
    }

    if (OptLevelO3 && OptLevelO3.getPosition() < PassList.getPosition(i)) {
      AddOptimizationPasses(M.get(), Passes, *FPasses, 3, 0);
      OptLevelO3 = false;
    }
...
#if defined(AMD_OPENCL) || 1
  // HSA_TODO: Perhaps one day we will handle -O0 just like other
  // optimization levels. For now, we roll a separate set of passes.
  // 
  ...
  if (OptLevelO0) {
    ...
    if (EnableGPUOpt)
      Passes.add(createAMDInlineAllPass(EnableWholeProgram));
    //Passes.add(createScalarReplAggregatesPass(-1)); 
  }
#endif
