...
public:
    ...
    void makeTest(Inst inst)
    {
        ...
        for (int i = provider->getFirstOperandIdx(); i <= provider->getLastOperandIdx(); i++)
        {
            ...
            else if (OperandVector reg = operand)
            {
                inst.operand(i) = getOperandVector(i);
            }
            else if (OperandImmed immed = operand)
            {
                inst.operand(i) = getOperandImmed(i);
            }
...
private:
    ...
    void emitInitCode()
    {
        for (int i = provider->getFirstSrcOperandIdx(); i <= provider->getLastOperandIdx(); ++i)
        {
            ...
            else if (OperandVector vec = operand)
            {
    ...
    void saveTestResults()
    {
        if (hasDstOperand())
        {
            ...
            else if (OperandVector(sampleDst))
            {
                OperandVector dst = getOperandVector(OPERAND_IDX_DST);
...
private:
    ...
    bool validateSrcData(unsigned operandIdx, Val v)
    {
        ...
        else if (OperandVector vec = opr)
        {
            ...
            {
                if (OperandWavesize(vec.operand(i)) && !isValidWsData(v[i])) return false;
            }
        }
...
private:
    ...
    OperandImmed getOperandImmed(unsigned idx) // Create i-th operand of test instruction
    {                                      
        OperandImmed immed = testSample.operand(idx);
        assert(immed);
    ...
    OperandVector getOperandVector(unsigned idx)  // Create register vector for i-th operand of test instruction.  
    {                                                   
        assert(0 <= idx && idx <= 4);
        assert(OperandVector(testSample.operand(idx)));

        OperandVector vec = testSample.operand(idx); // NB: this vector is read-only!
        ...
        HSAIL_ASM::ItemRange<Operand> opnds;
        for (unsigned i = 0; i < cnt; ++i) 
        {
            Operand opr;

            if (OperandReg x = vec.operand(i)) 
            {
                opr = context->emitReg(regSize, REG_IDX_VEC + i);
            }
            else if (OperandImmed x = vec.operand(i)) 
            {
                opr = context->emitImm(getImmSize(x), v[i].getAsB64(0), v[i].getAsB64(1));
            }
            else
            {
                assert(OperandWavesize(vec.operand(i)));
        ...
        OperandVector res = context->getContainer().append<OperandVector>();
        HSAIL_ASM::copy(res.operand(), opnds);

        return res;
    }

    static unsigned getVectorRegSize(OperandVector vec)
    {
        unsigned dim = vec.elementCount();

        for (unsigned i = 0; i < dim; ++i)
        {
            if (OperandReg x = vec.operand(i)) return getRegSize(x.reg());
        }
    ...
    {
        if (OperandVector vec = opr)
        {
            unsigned dim = vec.elementCount();

            for (unsigned i = 0; i < dim; ++i) if (!OperandReg(vec.operand(i))) return true;
        }
...
private:
    ...
    void initSrcVal(OperandVector vector, unsigned arrayIdx)
    {
        assert(vector);
        ...
        for (unsigned i = 0; i < dim; ++i)
        {
            if (OperandReg reg = vector.operand(i))
            {
                OperandAddress addr = context->emitAddrRef(addrReg, getSlotSize(regSize) / 8 * i);
                ldReg(regSize, context->emitReg(reg.reg()), addr);
            }
        }
    }
    ...
    void saveDstVal(OperandVector vector, unsigned arrayIdx)
    {
        assert(vector);
        ...
        {
            OperandAddress addr = context->emitAddrRef(addrReg, getSlotSize(regSize) / 8 * i);
            OperandReg reg = vector.operand(i);
            assert(reg); // dst vectors cannot include imm elements

            stReg(regSize, context->emitReg(reg.reg()), addr);
        }
    }
...
private:
    ...
    bool hasVectorOperand()
    {
        for (int i = 0; i < 5 && testSample.operand(i); ++i)
        {
            if (OperandVector(testSample.operand(i))) return true;
        }
        return false;
    }
...
private:
    ...
    static string   getName(OperandVector vector) 
    { 
        string res;
        for (unsigned i = 0; i < vector.elementCount(); ++i)
        {
            ...
            if (OperandReg reg = vector.operand(i))
            {
                res += static_cast<SRef>(reg.reg());
            }
            else if (OperandImmed imm = vector.operand(i))
            {
                res += "imm";
            }
            else
            {
                assert(OperandWavesize(vector.operand(i)));
                res += "ws";
            }
        }
        return "(" + res + ")";
    ...
    static unsigned getMaxDim(Inst inst)
    {
        ...
        {
            OperandVector vec = inst.operand(i);
            if (vec) return vec.operandCount();
        }
        return 1;
    }
...
private:
    ...
    static TestDataProvider* getProvider(Inst inst)
    {
        ...
        if (p)
        {
            ...
            for (int i = p->getFirstSrcOperandIdx(); i <= p->getLastOperandIdx(); ++i) 
            {
                ...
                unsigned dim  = (OperandVector(opr) || OperandAddress(opr))? maxDim : 1;
                bool     lock = !group || OperandImmed(opr) || OperandWavesize(opr) || isVectorWithImm(opr);
    ...
    static bool testableOperands(Inst inst)
    {
        ...
        for (int i = 0; i < 5; ++i)
        {
            ...
            else if (!OperandVector(operand) && !OperandReg(operand) && !OperandImmed(operand))
            {
                return false;
            }
        }
